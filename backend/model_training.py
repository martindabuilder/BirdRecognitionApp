...
# Trains the model using the training set and evaluates it after each epoch.
# According to the validation accuracy, the best model is saved.
# A saved checkpoint allows the model to continue training if inturrepted.
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, scaler, start_epoch=0, best_val_accuracy=-1.0, epochs_without_improvement=0):
    for epoch in range(start_epoch, EPOCHS):
        ...
        # The training data is processed batch by batch instead of loading the entire dataset at once.
        progress = tqdm(train_loader, desc = f"Epoch {epoch + 1}/{EPOCHS}")

        for images, labels, teacher_targets in progress:
            # The current batch is moved to the selected device (GPU or CPU).
...
def main():
    # If available, the GPu will be used for the training.
    device = torch.device(
        "cuda"
    ...
    # Prints the GPU that will be used.
    if device.type == "cuda":
    ...
    train_sampler = create_balanced_sampler(train_dataset, balance_power = BALANCE_POWER)
