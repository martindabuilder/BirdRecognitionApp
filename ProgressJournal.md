...
(editors note afterwards, but the approach got even deeper with more levels to it, so for a ful breakdown of that i suggest looking at the backend references readme file)
...
Day 18: Added spectrogram augmentation to 30% of the training split. It seems to have a payoff as the validation accuracy rose up to 55%, which is an improvement over the ~52% it was at before and this is before adding back the teacher distillation, for this run of experiments its turned off. The augmentation for now is only time and frequency masking, if the augmented training run gives good results i will consider adding more augmentation to another % of the set potentially or a second type to the already chosen 30%.
Overfitting is still present sadly so im doing my best to mitigate it in some way. One thing im considering is bringing down the class size to an even 200 or 210 (sitting at 212 at the moment, will depend on if i can help bring some classes a bit more samples that are on the lower side again) and reteaching the teacher with 50 (potentially higher) samples per class, that way its even more robust as i think it gave good improvements to the previous runs when it was with 30 files per class. In the case i decide to rerun the teacher i will start progress on the frontend side of things because as things stand the teacher might take another full day or two to train if its with 50+ samples per class, but ultimately it helps at the end so its better to wait out 2 days and have a good model, rather than rush finish it.

Day 19: Upped the teacher's files per class up to a 100 while i start working on the front end portion properly.
Working on the proper Python + FastAPI backend file.